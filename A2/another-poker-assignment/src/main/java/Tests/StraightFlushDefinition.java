...
public class StraightFlushDefinition {
	Player player;
	
	@Given("^A user has five cards in random order for straight$")
	public void a_user_has_five_cards_in_random_order_for_straight(DataTable data) {
	...
	public void a_Straight_flush_would_be_detected() {
		for (int i = 0; i < 5; i++) {
			Collections.shuffle(player.cards);

			System.out.println("The players hand is " + player.cards);
			Assert.assertEquals(Hand.STRAIGHT_FLUSH, player.getHighestHand());
			System.out.println("The players hand was a Straight Flush\n");
		}
	}
}
